...
const NewVote = () => {
  ...
  const calculateEndDate = (durationValue: string): Date => {
    ...
    const { data, error } = await supabase.from("polls").insert([
      {
        ...
        max_votes: maxChoices ? parseInt(maxChoices) : 1,
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-10">
      ...
      <CardFooter>
        <Button type="submit" className="w-full" onClick={(e: React.FormEvent<HTMLFormElement>) => handleSubmit(e)}>
          Créer le vote
        </Button>
      </CardFooter>
    </Card>
